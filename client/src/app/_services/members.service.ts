...
import { setPaginationHeaders, setPagintedResponse } from './pagination_helper';
...
export class MembersService {
  ...
  getMembers() {
    ...
    if (response) return setPagintedResponse(response, this.paginatedResult);
    ...
    return this.http.get<Member[]>(this.baseUrl + 'users', { observe: 'response', params })
      .subscribe({
        next: response => {
          setPagintedResponse(response, this.paginatedResult);
